...
import { CustomWord, Word } from '@/request/globals';
...
function PracticePageContent() {
  ...
  const loadPracticeData = useCallback(
    async (page: number = 1, pageSize: number = 10) => {
      ...
      const response = await getCustomPackageWords(
        packageId as string,
        page,
        pageSize
      );
      ...
      if (response && response.data && response.data.list) {
        const wordList = response.data.list.map(item => {
          return {
            ...item,
            category: item.customPackage.name,
            language: item.customPackage.description
          };
        });
        if (page === 1) {
          // 初始加载，替换所有数据
          setWords(wordList as unknown as CustomWord[]);
  ...
  }, [packageId, loadPracticeData, currentPage, pageSize]);
    ...
    async (_isCorrect: boolean) => {
      if (_isCorrect) {
        ...
        await createWordErrorRecord(
          words[currentWordIndex].customPackage.id.toString(),
          words[currentWordIndex].customPackage.description.toString(),
   ...
   * 切换到下一个单词的实际逻辑
   */
  const handleNextWordInternal = useCallback(() => {
   ...
   * 切换到下一个单词（带防抖处理）
   */
  const handleNextWord = useMemo(
    () => debounce(handleNextWordInternal, 300),
    [handleNextWordInternal]
  );

  /**
   * 切换到上一个单词（带防抖处理）
   */
  const handlePrevWord = useMemo(
    () => debounce(handlePrevWordInternal, 300),
    [handlePrevWordInternal]
  );
  ...
  return (
    <div
      ...
      <div className='flex flex-col'>
        ...
        <div className='w-screen mx-auto h-[60px] border-b relative flex items-center justify-between px-4 border-b-gray-500/30'>
          ...
          <div className='text-sm text-gray-300'>
            <span>
              {words?.[currentWordIndex]?.customPackage.description} -
              {words?.[currentWordIndex]?.customPackage.name} (
      ...
      <div className='flex-1 flex justify-center items-end mb-20'>
        <TypingText
          word={words[currentWordIndex] as unknown as Word}
          mode={currentMode}
